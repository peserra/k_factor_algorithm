...
Matching::Matching(const GrafoAdapter &G)
    : G(G), outer(2 * G.GetNumVertices()), deep(2 * G.GetNumVertices()),
      ...
      forest(2 * G.GetNumVertices()), root(2 * G.GetNumVertices()),
      blocked(2 * G.GetNumVertices()), dual(2 * G.GetNumVertices()),
      slack(G.GetNumEdges()), mate(2 * G.GetNumVertices()), m(G.GetNumEdges()),
...
void Matching::Grow() {
  ...
  // The forest is grown by extending a unmatched vertex w through a matched
  // edge u-v in a BFS fashion
  while (!forestList.empty()) {
    int w = outer[forestList.front()];
    forestList.pop_front();

    // w might be a blossom
    // we have to explore all the connections from vertices inside the blossom
    // to other vertices
    for (list<int>::iterator it = deep[w].begin(); it != deep[w].end(); it++) {
      int u = *it;

      int cont = false;
      for (list<int>::const_iterator jt = G.AdjList(u).begin();
           jt != G.AdjList(u).end(); jt++) {
        int v = *jt;
...
bool Matching::IsEdgeBlocked(int e) { return GREATER(slack[e], 0); }

// Vertices will be selected in non-decreasing order of their degree
// Each time an unmatched vertex is selected, it is matched to its adjacent
// unmatched vertex of minimum degree
void Matching::Heuristic() {
  ...
  while (B.Size() > 0) {
    ...
    if (mate[outer[u]] == -1) {
      int min = -1;
      for (list<int>::const_iterator it = G.AdjList(u).begin();
           it != G.AdjList(u).end(); it++) {
        int v = *it;

        if (IsEdgeBlocked(u, v) or (outer[u] == outer[v]) or
            (mate[outer[v]] != -1))
          continue;
...
void Matching::DestroyBlossom(int t) {
  ...
  for (list<int>::iterator it = shallow[t].begin(); it != shallow[t].end();
       it++) {
    int s = *it;
    outer[s] = s;
...
void Matching::Expand(int u, bool expandBlocked = false) {
  ...
  // We use the minimum index to grant that the two possible blossoms u and v
  // will use the same edge for a mate
  ...
  for (list<int>::iterator it = shallow[u].begin();
       it != shallow[u].end() and not found;) {
    int si = *it;
    for (list<int>::iterator jt = deep[si].begin();
         jt != deep[si].end() and not found; jt++) {
      if (*jt == p)
        found = true;
    }
    it++;
  ...
  // We update the sets blossom, shallow, and outer since this blossom is being
  // deactivated
  for (list<int>::iterator it = shallow[u].begin(); it != shallow[u].end();
       it++) {
    int s = *it;
    outer[s] = s;
    for (list<int>::iterator jt = deep[s].begin(); jt != deep[s].end(); jt++)
...
void Matching::AddFreeBlossomIndex(int i) { free.push_back(i); }
...
// Contracts the blossom w, ..., u, v, ..., w, where w is the first vertex that
// appears in the paths from u and v to their respective roots
int Matching::Blossom(int u, int v) {
  ...
  for (list<int>::iterator it = shallow[t].begin(); it != shallow[t].end();
    ...
    u_ = *it;
    outer[u_] = t;
    for (list<int>::iterator jt = deep[u_].begin(); jt != deep[u_].end();
         jt++) {
      deep[t].push_back(*jt);
      outer[*jt] = t;
    }
...
void Matching::UpdateDualCosts() {
  ...
  for (int i = 0; i < m; i++) {
    ...
    } else if ((outer[u] != outer[v]) and type[outer[u]] == EVEN and
               type[outer[v]] == EVEN) {
  ...
  for (int i = n; i < 2 * n; i++) {
    if (active[i] and i == outer[i] and type[outer[i]] == ODD and
        (!inite3 or GREATER(e3, dual[i]))) {
      e3 = dual[i];
      inite3 = true;
    }
  }
...
Matching::SolveMinimumCostPerfectMatching(const vector<double> &cost) {
